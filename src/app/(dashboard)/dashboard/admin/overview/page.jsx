    ...
    return (
      <div className="w-full min-h-screen flex flex-col items-center justify-center p-4 md:p-8 bg-slate-50">
        <div className="w-full max-w-md bg-white border border-slate-200 p-6 md:p-8 flex flex-col gap-4 text-center shadow-sm">
          ...
          <Link href="/" className="mt-2 px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition cursor-pointer shadow-sm">
            Sign In
          </Link>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200 p-5 md:p-6 shadow-sm flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <Link href="/dashboard/admin/sales" className="text-xs font-bold text-slate-800 hover:text-slate-600 flex items-center gap-0.5 transition">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {recentOrders.map(order => (
                      <tr key={order.order_id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-2.5 sm:px-3 py-2.5 text-center">
                          <Link href={`/dashboard/sales/sale/${order.order_id}`} className="px-2.5 py-1 bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-bold transition cursor-pointer">
                            Details
                          </Link>
                        </td>
                      </tr>
          ...
          <div className="bg-white border border-slate-200 p-5 md:p-6 shadow-sm flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 gap-2.5">
              <Link href="/dashboard/admin/people" className="p-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition font-bold text-xs text-slate-700 flex items-center justify-between group">
                <span>Accounts & Staff Roles</span>
                <BiChevronRight className="text-base text-slate-400" />
              </Link>
              <Link href="/dashboard/admin/stock" className="p-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition font-bold text-xs text-slate-700 flex items-center justify-between group">
                <span>Warehouse Stock Inventory</span>
                <BiChevronRight className="text-base text-slate-400" />
              </Link>
              <Link href="/dashboard/admin/sales" className="p-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition font-bold text-xs text-slate-700 flex items-center justify-between group">
                <span>Global Sales Desk</span>
                <BiChevronRight className="text-base text-slate-400" />
              </Link>
              <Link href="/dashboard/admin/payments" className="p-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition font-bold text-xs text-slate-700 flex items-center justify-between group">
                <span>Payments Audit Ledger</span>
                <BiChevronRight className="text-base text-slate-400" />
              </Link>
              <Link href="/dashboard/admin/report" className="p-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition font-bold text-xs text-slate-700 flex items-center justify-between group">
